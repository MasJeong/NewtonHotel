	...
	<div class="wrap">
		...
		<div class="listContainer">
			<div
					...
					<form name="search" method="post" action="ReserServlet?command=search_action">
						<fieldset>
							...
							<select name="keyField">
								<option ${(param.keyField == "userId")? "selected" : "" } value="userId">아이디</option>
								<option ${(param.keyField == "userName")? "selected" : "" } value="userName">이름</option>
							</select> 
							<label>검색어</label> 
							<input type="text" name="keyWord" size="40" value="${param.keyWord }" class="input1"> 
				...
				<table class="list" border="1" style="text-align: center">
					...
					<c:forEach var="reser" items="${reserList }">
						<tr class="record">
							...
							<td><fmt:parseDate value='${reser.reservation_in}'
									var='reservation_in' pattern='yyyy-MM-dd' /> <fmt:formatDate
									pattern="yyyy-MM-dd" value="${reservation_in }" /></td>
							<td><fmt:parseDate value='${reser.reservation_out}'
									var='reservation_out' pattern='yyyy-MM-dd' /> <fmt:formatDate
				...
				<br>
				<p>
				<input type="button" class="button" value="모든목록 조회" onclick="location.href='ReserServlet?command=all_user_reser_select'" style="float: right;">
				</p>
			</div>
		</div>
	</div>
