		...
		<table align="center" class="table">
			<tr>
				...
				<td><label for="reservation_in"> 예약 날짜: </label>
					<fmt:formatDate pattern="yyyy-MM-dd" value="${reservation_in }"/>~
					<fmt:formatDate pattern="yyyy-MM-dd" value="${reservation_out }"/>
				</td>
			</tr>
			<tr>
				<td><label for="adult"> 성인: </label>
					${reserInfo.adult} 명</td>
				<td><label for="child"> 어린이: </label>
					${reserInfo.child} 명</td>
			</tr>
			<tr>
				<td><label for="hotelName"> 호텔명: </label>
					${reserInfo.hotelName }</td>
				<td><label for="price"> 가격: </label>
					 ${reserInfo.price } 원</td>				
			</tr>
			<tr>
				<td><label for="roomType"> 룸 타입: </label> 
					${reserInfo.roomType }</td>
				<td><label for="reservation_number"> 예약 번호: </label> 
					${reserInfo.reservation_number }</td>	
			</tr>
		</table>
		
		<br> <p class="fs-1" align="center">예약 취소는 '마이페이지'에서 확인해 주십시오.</p> 
			<input type="button" class="button1" value="메인 페이지로 이동" onclick="location.href='main.jsp'">
		</div>
		<br><br>
