		...
		<table class="list">
			<tr>
				<td colspan="5" style="border: white; text-align: right"><c:if
						...
						<a href="/newton_hotel/login.do" onclick="message()">후기 등록</a>
					</c:if> <c:if test="${user_id ne null}">
						<a href="reviewsServlet?command=reviews_write_change_window">후기 등록</a>
					</c:if></td>
...
<%-- 					<td>${reviews.password }</td> --%>
					<td>${reviews.title }</td>
					<td>${reviews.content }</td>
					<td>${reviews.readcount }</td>
					<td><a href="reviewsServlet?command=reviewsView&num=${reviews.num}">자세히보기</a></td>
